...
<!-- 
	...
	로그인 기능을 구현 하는데 사용됩니다.
		...
		클라이언트에서 서버로 페이지를 요청하면 서버는 페이지를 응답해주고 토인은 종료되는 것이다.
		
		서브는 어떻게 로그인 상태를 유지시켜 줄수 있을까요?
		
		클라이언트에서 웹브라우저로 서버에 최초로 접근시
			-> 웹서버는 웹브라우저에게 세션아이디를 발급
			-> 세션아이디는 쿠키로 생성되어 응답객체의 쿠키에 담겨 클라이언트로 전송
				개발자도구의 쿠키를 확인해보면 HSESESSIONID라는 값이 입력되어 있습니다.
				여기 저장된 값이 세션 아이디 입니다.
			(웹브라우저를 새로 열고 서버에 접근시 쿠키에 저장된 세션ID값이 변경된것을 확인할 수 있습니다.)
	...
	세션의 유지시간
		...
		클라이언트에서 쿠키를 제거할 경우 세션아이디도 함께 제거 되므로 서버는 새로운 세션아이디를 발급 합니다.
		
	유지시간 설정 방법
		web.xml : 유지시간을 분단위로 설정할 수 있습니다.
					유지시간 설정 후 웹브라우저를 닫았다가 다시 열어야 확인 가능!
		session.setMaxInactiveInterval() : 유지시간을 초단위로 설정할 수 있습니다.
 -->
 
 <%
 
 	// session.setMaxInactiveInterval(100); // 세션 유지시간 설정 초단위
 ...
 <ul>
 	...
 	<li>세션 유치 시간 : <%=session.getMaxInactiveInterval() %></li>
 </ul>
