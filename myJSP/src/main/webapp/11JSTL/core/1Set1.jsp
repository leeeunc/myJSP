...
<body>
	...
	<!-- 
	JSTL (JSP Standera Tag Library)
		JSP의 표준태그라이브러리를 이용하면 스크립틀릿을 사용하지 않고 태그를 사용하여
		제어문, 반복문등을 사용할 수 있습니다.
		
		기본태그가 아닌 확장태그 이므로 사용하기 위해서는 별도의 라이브러리가 필요합니다.
		메이븐리파지토리로 접근하여 jstl-1.2.jar 파일을 다운로드 받고 lib폴더에 추가합니다.
		
		JSTL 종류
			Core태그 : 변수선언, 조건문/반복문, URL처리
			접두어 : c
			
			Foramtting 태그 : 숫자, 날짜, 시간 포맷 지정
			접두어 : fmt
...
<!-- 
 ...
 -->
 	...
 	<c:set var="betweenVar"> 변수값 요렇게 설절 </c:set>
	...
	<ul>
		...
		<li>아이디 : ${requestScope.map.pArgs1.id }</li> <!-- scope로 접근 -->
	</ul>
	 
	<ul>
		<li>이름 : ${map.pArgs2.name }</li>
		<li>아이디 : ${map.pArgs2.id }</li>
		<li>아이디 : ${requestScope.map.pArgs2.id }</li> <!-- scope로 접근 -->
	</ul>
